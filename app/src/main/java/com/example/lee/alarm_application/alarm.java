 ...
 * Description: 기본 알람 관련 컨텐트들이 들어있는
 * 엑티비티 이다. 이곳에서는 알람의  snooze 와  알람의 해제를 사용할수 있다.
 */
public class alarm  extends AppCompatActivity {
    ...
    BroadcastReceiver AlarmReceiver1;
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        finish = (Button) findViewById(R.id.finishalarming4);
        alarmpush = (Button) findViewById(R.id.alarmingpush);
        presenttime1 = (TextView) findViewById(R.id.presenttime);
        getmessage = (TextView) findViewById(R.id.message12);
        clock=(ImageView)findViewById(R.id.alarmclockimage);
        ...
        @SuppressLint("InvalidWakeLockTag") final PowerManager.WakeLock wakeLock= pm.newWakeLock(PowerManager.FULL_WAKE_LOCK, "POWER");
        wakeLock.acquire();
        ...
        alarm_pushed = intentalarm.getBooleanExtra("알람미루기1", false);
        get_message = intentalarm.getStringExtra("알람메모임");
        get_alarm_kinds=intentalarm.getStringExtra("알람소리종류1");
        alarm_date_using=intentalarm.getBooleanExtra("요일여부1",false);
        get_alarm_position = intentalarm.getIntExtra("알람날짜미루기용1", -1);
       ...
       if(get_message.equals("")||get_message==null){
           getmessage.setVisibility(View.INVISIBLE);
            ...
            getmessage.startAnimation(translate);
        }
        ...
        if (alarm_pushed == true) {
            alarmpush.setVisibility(View.VISIBLE);
        } else if (alarm_pushed == false) {
            alarmpush.setVisibility(View.INVISIBLE);
        }
        ...
        //C:\Users\leedonghun\AppData\Local\Android\Sdk
        //알람 미루기  버튼을 눌렀을때 진행되는 사항
        alarmpush.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
       ...
       AlarmReceiver1=new BroadcastReceiver() {
            @Override
        ...
        registerReceiver(AlarmReceiver1,filter);
